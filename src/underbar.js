(function() {
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(passed, element, index, collection) {
      if(!passed) {
        return false;
      }
      return Boolean((iterator || _.identity)(element, index, collection));
    }, true);
  };
  ...
  _.shuffle = function(array) {
    ...
    var shuffledResults = [];
    var randomIndex;
    while(shuffledResults.length < array.length) {
      randomIndex = Math.floor(Math.random() * copy.length);
      shuffledResults = shuffledResults.concat(copy.splice(randomIndex, 1));
    }
